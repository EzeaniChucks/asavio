...
export default function AdminHotelsPage() {
  ...
  const reject = async () => {
    ...
    try {
      await api.patch(`/admin/hotels/${rejectTarget.id}`, {
        status: "rejected",
  ...
  const toggleStarVerified = async (h: Hotel) => {
    setActionLoading(h.id + "-star");
    try {
      await api.patch(`/admin/hotels/${h.id}`, {
        verifiedStarRating: !h.verifiedStarRating,
      });
      toast.success(h.verifiedStarRating ? "Star rating unverified" : "Star rating verified");
      fetchHotels();
    } catch {
      // interceptor handles
    } finally {
  ...
  return (
    <AdminPageGuard permission={P.MANAGE_HOTELS}>
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Hotels moderation</h1>
          ...
          {isLoading ? (
            ...
            <div className="space-y-3">
              <AnimatePresence>
                {hotels.map((h) => {
                  const primary = h.images?.find((i) => i.isPrimary)?.url ?? h.images?.[0]?.url;
                  const statusClass =
                  ...
                  return (
                    ...
                    >
                      <div className="relative w-24 h-24 rounded-xl overflow-hidden flex-shrink-0 bg-gray-100">
                        {primary ? (
                          <Image src={primary} alt={h.name} fill className="object-cover" />
                        ) : (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                          ...
                          {!h.isAvailable && h.status === "approved" && (
                        ...
                        <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-gray-500 mb-2">
                          ...
                          {h.host && (
                            <span>
                              Host: {h.host.firstName} {h.host.lastName}
                            </span>
                          )}
                        </div>
                        ...
                        {typeof h.starRating === "number" && h.starRating > 0 && (
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <button
                              onClick={() => toggleStarVerified(h)}
                              disabled={actionLoading === h.id + "-star"}
                            ...
                            >
                              {h.verifiedStarRating ? <FaCheckCircle className="text-[10px]" /> : null}
                              {h.verifiedStarRating ? "Verified" : "Verify stars"}
                            </button>
                          </div>
                        )}

                        {h.status === "rejected" && h.rejectionReason && (
                          <p className="text-xs text-red-500 italic mb-2">Reason: {h.rejectionReason}</p>
                        )}
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          {h.status !== "rejected" && (
                            <button
                              onClick={() => { setRejectTarget(h); setRejectionReason(""); }}
      ...
      <AnimatePresence>
        {rejectTarget && (
          ...
          >
            ...
            >
              <h2 className="font-semibold text-gray-900 mb-1">Reject &quot;{rejectTarget.name}&quot;?</h2>
              <p className="text-xs text-gray-400 mb-4">Host will be notified with your reason.</p>
              <textarea
              ...
              <div className="flex gap-2">
                ...
                >
                  {actionLoading === rejectTarget.id + "-reject" ? "Rejecting…" : "Reject"}
                </button>
              </div>
            </motion.div>
          </motion.div>
        ...
        {deleteTarget && (
          ...
          >
            <motion.div
              ...
              onClick={(e) => e.stopPropagation()}
            >
              <h2 className="font-semibold text-gray-900 mb-3">Delete this hotel?</h2>
              <p className="text-sm text-gray-500 mb-5">
                This permanently removes the hotel, all its rooms, and all its images. Cannot be undone.
              </p>
              <div className="flex gap-2">
                <button
                  onClick={() => setDeleteTarget(null)}
                ...
                <button
                  onClick={() => deleteHotel(deleteTarget)}
